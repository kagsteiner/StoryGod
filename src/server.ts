...
const applicationBasePath = "/aidventure";
...
export const server = createServer(async (req, res) => {
  ...
  const pathname = url.pathname === applicationBasePath ? "/" : url.pathname.startsWith(`${applicationBasePath}/`) ? url.pathname.slice(applicationBasePath.length) : url.pathname;
  try {
    if (method === "GET" && pathname === "/api/health") return json(res, 200, { ok: true, mode: "openai", models: configuredAIModels(), configured: openAIConfigured() });
    if (method === "POST" && pathname === "/api/candidates") { const payload = await body(req) as { tasteProfile?: unknown }; const taste = normalizeTaste(payload.tasteProfile ?? payload); return json(res, 200, { candidates: await generateCandidatesWithAI(taste), tasteProfile: taste, model: modelForTask("genome") }); }
    if (method === "POST" && pathname === "/api/validate") { const payload = await body(req) as Partial<GenomeCandidate>; if (!payload.storyGenome || !payload.worldGenome) throw new Error("storyGenome and worldGenome are required."); return json(res, 200, validateGenome(payload.storyGenome, payload.worldGenome)); }
    if (method === "POST" && pathname === "/api/repair") { const payload = await body(req) as Partial<GenomeCandidate>; if (!payload.storyGenome || !payload.worldGenome) throw new Error("storyGenome and worldGenome are required."); return json(res, 200, applyRepairs(payload.storyGenome, payload.worldGenome)); }
    if (method === "POST" && pathname === "/api/score") { const payload = await body(req) as { tasteProfile?: unknown; candidate?: GenomeCandidate }; if (!payload.candidate) throw new Error("candidate is required."); const taste = normalizeTaste(payload.tasteProfile); return json(res, 200, scoreGenome(taste, payload.candidate.storyGenome, payload.candidate.worldGenome)); }
    if (method === "POST" && pathname === "/api/project") { const payload = await body(req) as { tasteProfile?: unknown; candidate?: GenomeCandidate }; if (!payload.candidate) throw new Error("candidate is required."); return json(res, 200, { project: await createProjectWithAI(normalizeTaste(payload.tasteProfile), payload.candidate), models: { architecture: modelForTask("architecture"), bibles: modelForTask("bibles") } }); }
    if (method === "POST" && pathname === "/api/projects") { const payload = await body(req) as { tasteProfile?: unknown; candidate?: GenomeCandidate }; if (!payload.candidate) throw new Error("candidate is required."); const job = await createProjectJob(normalizeTaste(payload.tasteProfile), payload.candidate); startProjectGeneration(job.id); return json(res, 202, { job: publicProjectJob(job) }); }
    ...
    if (storyRoute) {
      ...
      if (method === "GET" && action === "export") { const session = await getStorySession(id); if (!session) return json(res, 404, { error: "Story session not found." }); const markdown = Buffer.from(storyMarkdown(session), "utf8"); res.writeHead(200, { "content-type": "text/markdown; charset=utf-8", "content-disposition": `attachment; filename="${storyMarkdownFilename(session.title)}"`, "content-length": markdown.length }); res.end(markdown); return; }
    ...
    if (method === "GET") return await staticFile(pathname, res);
